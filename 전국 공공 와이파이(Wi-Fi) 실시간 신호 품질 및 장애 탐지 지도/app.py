...
# 1. 파일 경로 설정 및 데이터 로드
# 배포 환경에서 경로 에러가 나지 않도록 절대 경로 구조를 명확히 잡습니다.
...
def load_and_process_data():
    if not os.path.exists(file_path):
        st.error(f"데이터 파일을 찾을 수 없습니다. 경로를 확인해주세요: {file_path}")
        return pd.DataFrame()
...
if not df_processed.empty:
    ...
    # 지도에 최대 100개 마커만 표시 (브라우저 메모리 최적화 및 배포 환경 로딩 속도 방지)
